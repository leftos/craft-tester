...
- [ ] 14. UI: strip, ATIS, CRAFT form, results, seed in URL hash — landed 2026-09-15 and steered through 2026-09-16 (archive: result rows, revisit spoiler, expect-runway element, time and configuration filters, C dropped, clean draws and R.sid dropped, "(no prefix)" shape, read-only CRAFT rows, published-top "climb via SID", the acceptable-but-inefficient verdict). Left:
  ...
  - [ ] **User steer 2026-09-16**: add "expect filed altitude 5 minutes after departure" as a second distractor in the expect dropdown (`PlayerPicks.expect` gains `five_minutes`; never the engine's answer unless a row says 5; the stored-picks schema in `solved.ts` accepts it) (landed 2026-09-16; the dropdown order is 10, 5, 3, none; `EXPECT_CHOICES` is exported from `options.ts` and shared with `state.ts`)
...
- [ ] 21. Validation loop, amendment mode: 52 worksheet amendment plans, one at a time with the user, as step 17 was. **Settled 3/52 on 2026-09-16** (Practice 1A: AAY218 FL360 → FL350 parity; JSX201 WESLA5 → SSTIK5 off 01L; KAL65 no SID → SFO5 RBL off 01R, truncated tail kept as transcribed); next: the rest of Practice 1A (LXJ351 as filed, N172SP, N238JP, SKW2345, SWA126, SWA1859, SWA1883, SWA1984, SWA984, XOJ715), then 1C, 2, 3. SWA2021 stays unresolved since its KPDX `DEDHD LMT OCITY#` route left the library (the Portland LOA never allowed it)
