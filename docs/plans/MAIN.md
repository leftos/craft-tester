...
- [ ] 17. Validation loop, clearance mode (one at a time with the user, then batch). `pnpm -C web propose <id>` and `--pending` landed. Questions for the user, from the synthetic fixtures and the worksheet table:
  ...
  - [ ] User steer 2026-09-15 (N483KA): in 28/01, GA aircraft up to light jets depart 28R at intersection E (short taxi from the north ramp). Model as a `default_for_classes: [P, T]` flag on the 28R departure-runway row of the 28/01 config; the importer and the scenario generator prefer that runway for those classes before the direction split. Jets stay on the 01s until a GA flag exists on the fleet (ask the user whether GA jets should follow)
