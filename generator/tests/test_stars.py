...
from craft_generator.cifp.stars import CifpStar, StarRecord, parse_star_record, parse_stars
...
def _ids(stars: tuple[CifpStar, ...]) -> frozenset[str]:
    return frozenset(star.id for star in stars)


def test_seattle_publishes_exactly_its_five_arrivals(destination_stars: dict[str, tuple[CifpStar, ...]]) -> None:
    assert _ids(destination_stars["KSEA"]) == KSEA_ARRIVALS
...
def test_an_identifier_loses_its_padding(star_record_lines: list[str]) -> None:
    ...
    assert parse_star_record(row) == StarRecord(airport="KSEA", star_id=SHARED_ARRIVAL, route_type="1", transition="BTG")


def test_an_arrival_carries_its_family_rnav_flag_and_enroute_transitions(destination_stars: dict[str, tuple[CifpStar, ...]]) -> None:
    hawkz = next(star for star in destination_stars["KSEA"] if star.id == SEATTLE_ONLY_ARRIVAL)
    olm = next(star for star in destination_stars["KSEA"] if star.id == SHARED_ARRIVAL)
    assert (hawkz.family, hawkz.rnav) == ("HAWKZ", True)
    assert (olm.family, olm.rnav) == ("OLM", False)
    assert "BTG" in olm.transitions


def test_an_arrival_serving_two_airports_is_listed_under_both(destination_stars: dict[str, tuple[CifpStar, ...]]) -> None:
    assert SHARED_ARRIVAL in _ids(destination_stars["KSEA"])
    assert SHARED_ARRIVAL in _ids(destination_stars["KBFI"])


def test_boeing_field_does_not_publish_the_seattle_arrival(destination_stars: dict[str, tuple[CifpStar, ...]]) -> None:
    assert SEATTLE_ONLY_ARRIVAL in _ids(destination_stars["KSEA"])
    assert SEATTLE_ONLY_ARRIVAL not in _ids(destination_stars["KBFI"])


def test_a_foreign_destination_is_not_in_the_faa_file_at_all(
    destination_stars: dict[str, tuple[CifpStar, ...]], ksfo_airport_records: dict[str, AirportRecord]
) -> None:
...
def test_a_continuation_row_is_skipped(star_record_lines: list[str]) -> None:
    ...
    assert parse_stars([continuation]) == {}
