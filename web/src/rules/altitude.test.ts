...
describe('the expect clause', () => {
  ...
  it('follows an interim altitude below the filed one while the toggle is conditional', () => {
    ...
    expect(
      resolve(ctx({}), sid('SEGUL1'), scenario({ filedAltitude: 10000 }), airport).expect.value,
    ).toBeNull();
  });
});
